...
#   從 script/Quant/ 目錄執行：
#
#   FP32 checkpoint → FP16 inference (TRT quantization, default):
#     RUN_FAMILY=sic_int \
#     RUN_CHECKPOINT=/raid/jess/SICNet/out_sic_small_int/20250527-172928-sic_small_patch4_256_int_win-256/model_best.pth.tar \
#     bash run_quant.sh
#
#   FP32 checkpoint → FP32 inference (PyTorch, no quantization):
#     RUN_CKPT_DTYPE=fp32 RUN_INFER_DTYPE=fp32 \
#     RUN_FAMILY=sic_int \
#     RUN_CHECKPOINT=/raid/jess/.../model_best.pth.tar \
#     bash run_quant.sh
#
#   FP32 checkpoint → INT8 inference (TRT INT8 quantization):
#     RUN_INFER_DTYPE=int8 \
#     RUN_FAMILY=sic_int \
#     RUN_CHECKPOINT=/raid/jess/.../model_best.pth.tar \
#     bash run_quant.sh
#
#   Native FP16 checkpoint → FP16 inference (PyTorch direct, no TRT):
#     RUN_CKPT_DTYPE=fp16 RUN_INFER_DTYPE=fp16 \
#     RUN_FAMILY=sic_int \
#     RUN_CHECKPOINT=/path/to/fp16_model_best.pth.tar \
#     bash run_quant.sh
#
...
RUN_OUT="${RUN_OUT:-${ROOT}/results/Quant/quant_${RUN_FAMILY}_${RUN_CKPT_DTYPE}to${RUN_INFER_DTYPE}.csv}"
...
# ---- checkpoint 檢查 ----------------------------------------
if [[ "${RUN_TASK}" != "bench" && -z "${RUN_CHECKPOINT}" ]]; then
  echo "[ERR] RUN_CHECKPOINT must be set, e.g.:" >&2
  echo "[ERR]   RUN_CHECKPOINT=/raid/jess/.../model_best.pth.tar bash run_quant.sh" >&2
  exit 1
fi
...
mkdir -p "${RUN_LOG_DIR}" "${ROOT}/results/Quant" "${RUN_ENGINE_DIR}"
